...
func eventEditServer(t *testing.T) *httptest.Server {
	t.Helper()
	return httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		switch {
		case r.Method == "GET" && r.URL.Path == "/calendars.json":
				...
				"Calendar::Event": []map[string]any{
					{
						...
						"title":               "Original",
						"all_day":             false,
						"starts_at":           "2026-04-06T10:00:00-03:00",
						"ends_at":             "2026-04-06T11:00:00-03:00",
						"starts_at_time_zone": "Brasilia",
						"ends_at_time_zone":   "Brasilia",
						"type":                "Calendar::Event",
						"calendar":            map[string]any{"id": 1},
					},
				},
			}
			w.Header().Set("Content-Type", "application/json")
		...
		case r.Method == "PATCH" && r.URL.Path == "/calendar/events/100":
			w.Header().Set("Location", "https://app.hey.com/calendar/days/2026-04-06")
			w.WriteHeader(302)
...
func runEventEdit(t *testing.T, server *httptest.Server, args ...string) (output.Response, error) {
	t.Helper()
	t.Setenv("HEY_TOKEN", "test-token")
	t.Setenv("HEY_NO_KEYRING", "1")
	t.Setenv("HEY_BASE_URL", "")
	...
	root.SetOut(&buf)
	root.SetErr(&buf)
	root.SetArgs(append([]string{"event", "edit", "--json", "--base-url", server.URL}, args...))
...
func TestEventEdit(t *testing.T) {
	server := eventEditServer(t)
	defer server.Close()

	resp, err := runEventEdit(t, server, "100", "--start", "14:00", "--end", "15:00")
	if err != nil {
		t.Fatalf("execute: %v", err)
	}
	if !resp.OK {
...
func TestEventEdit_Title(t *testing.T) {
	server := eventEditServer(t)
	defer server.Close()

	resp, err := runEventEdit(t, server, "100", "--title", "Updated Title")
	if err != nil {
		t.Fatalf("execute: %v", err)
	}
	if !resp.OK {
